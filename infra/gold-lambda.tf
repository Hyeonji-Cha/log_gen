...
data "archive_file" "gold_lambda" {
  type        = "zip"
  source_file = "${path.module}/../lambda/gold/main.py"
  output_path = local.gold_lambda_zip
}
...
resource "aws_lambda_event_source_mapping" "silver_to_gold" {
  ...
  # 어떤 데이터부터 읽어서 처리하는 가(LATEST, TRIM_H..)
  starting_position = "LATEST"
  # 한번에 lambda에서 호출하는 최대 레코드 개수
  batch_size = 100 # 100개 읽어서 처리(실시간보다는 배치작업성이 높은 구조)
  # 트래픽 상승 대비하여 대기시간 부여
  maximum_batching_window_in_seconds = 5
  # 테라폼을 구성한 이후 활성화
  enabled = true
  # 실패한 레코드에 대해 성공레코드도 섞여 있을 경우, 다시 처리 할 것인가? -> 다시 처리 안함
