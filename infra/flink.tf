...
resource "aws_kinesisanalyticsv2_application" "silver" {
  ...
  # 앱: true면 즉시 실행, false 생성만 하고 실행은 직접 세팅
  ...
  application_configuration {
    ...
    environment_properties {
      ...
      property_group {
        # AWS Managed Flink를 인식하는 예약 그룹명(실행옵션)-> 고정값
        property_group_id = "kinesis.analytics.flink.run.options"

        property_map = {
          # flink 앱의 엔트리 포인트
          "python" = "main.py"
          # pyflink에서 kinesis접근 -> 드라이브(라이브러리)필요-> *.jar
          "jarfile" = "lib/pyflink-dependencies.jar"
          # python UDF Worker가 import하도록 등록
    ...
    flink_application_configuration {
      ...
      monitoring_configuration {
        ...
        log_level          = "INFO"        # 정보수준
        metrics_level      = "APPLICATION" # 애플리케이션 레벨 정보
      }
      # 애플리케이션 구동시 병렬 처리, KPU 설정
      parallelism_configuration {
        configuration_type   = "CUSTOM"                      # 직접설정
  ...
  # flink가 실행 됐을때 나오는 로그(정상, 에러)=> cloudwatch 저장
